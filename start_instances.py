...
def start_stopped_instances(compartments, regions, config):
    """ Start stopped instances in each compartment for each region """
    for compartment in compartments:
        ...
        for region in regions:
            ...
            try:
                stopped_instances = compute_client.list_instances(
                    compartment.id, lifecycle_state='STOPPED'
                ).data
            except Exception as e:
            ...
            if stopped_instances:
                print(f"  Found {len(stopped_instances)} stopped instances in {compartment.name}")
                    ...
                    if ('Dev' in instance.freeform_tags and instance.freeform_tags['Dev'] == 'Yes') or \
                       ('Test' in instance.freeform_tags and instance.freeform_tags['Test'] == 'Yes'):
                        ...
                        try:
                            ...
                            print(f"    Starting instance: {instance.display_name}")
                        except Exception as e:
                            print(f"    Error starting instance {instance.display_name}: {str(e)}")
            else:
                print(f"  No stopped instances found in {compartment.name} for region {region}")
...
if __name__ == "__main__":
    ...
    # Start stopped instances in the active compartments for each region
    start_stopped_instances(active_compartments, subscribed_regions, config)
